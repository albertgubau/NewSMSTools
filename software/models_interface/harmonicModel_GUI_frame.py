...
class HarmonicModel_frame:
    ...
    def compute_model(self):
        ...
        try:
            ...
            f0et = int(self.f0et.get())  # Frequency deviation offset?
            ...
            w = es.Windowing(type=window)
            fft = es.FFT(size=N)
            pitcher = es.PitchYin()
            harmonicAnal = es.HarmonicModelAnal(sampleRate=fs,
            ...
            for frame in es.FrameGenerator(audio=x, frameSize=N, hopSize=H):
                ...
                # Analysis
                h_anal = harmonicAnal(infft, pitch[0])
                

                # Synthesis
                fft_synth = sineSynth(h_anal[1], h_anal[0], h_anal[2])

                if frames == 0:  # First frame
                    hfreq = np.array([h_anal[0]])

                else:  # Next frames
                    hfreq = np.vstack((hfreq, np.array([h_anal[0]])))
